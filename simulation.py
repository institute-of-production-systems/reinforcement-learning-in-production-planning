...
from rl.common import load_environment
...
class ProductionSystemSimulation(AbstractGame):
    '''
    Provides a production system "game" that MuZero or other RL algorithms can play. Makes use of Gym environments.
    '''
    ...
    def legal_actions(self):
        ...
        leg_act = self.env.get_legal_actions()

        if leg_act == [-1]:
            print('simulation.py: legal_actions() returns [0]... This should only be the case once the simulation is finished!')
            return [0]
        else:
            return leg_act
