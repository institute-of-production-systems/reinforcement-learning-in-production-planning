...
Trained RL models can be used to simulate new runs based on the optimized data. In order to do so you have to save the checkpoint file as well as the action/observation space configuration files. It is important to note that this will only work if your previously trained and optimized runs have the exact same production system as the one you want to run it on now. Now to actually load your trained models you need to click the 'Load and use model...' button and open your checkpoint file. 
An example for this is presented in the 'load_rl_example' folder. This is a previously optimized run based on our example production system.

Following you can see gantt charts demonstrating a schedule generated by heuristics and by randowm decisions.
